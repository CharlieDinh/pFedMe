...
def plot_summary_one_figure(num_users=100, loc_ep1=5, Numb_Glob_Iters=10, lamb=[], learning_rate=[], alpha=[], algorithms_list=[], batch_size=0, dataset = "", k = [], personal_learning_rate = []):
    ...
    plt.ylim([0.8, train_acc.max() + 0.01])
    ...
    plt.ylim([0.8,  glob_acc.max() + 0.01])
...
def plot_summary_one_figure_mnist(num_users, loc_ep1, Numb_Glob_Iters, lamb, learning_rate, alpha, algorithms_list, batch_size, dataset, k, personal_learning_rate):
    ...
    for i in range(Numb_Algs):
        label = get_label_name(algorithms_list[i])
        plt.plot(train_loss[i, 1:], linestyle=linestyles[i], label=label, linewidth = 1, color=colors[i],marker = markers[i])
    plt.legend(loc='upper right')
    plt.ylabel('Training Loss', size=14)
    plt.xlabel('Global rounds', size=14)
    plt.ylim([0.195,  0.5])
